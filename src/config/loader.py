...
try:
    ...
    raise RuntimeError(f"❌ CRITICAL: MongoDB dependencies missing - {e}. Install pymongo: pip install pymongo")
...
def _load_json_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"❌ Invalid JSON format in {path}: {exc}")
    except Exception as exc:
        raise RuntimeError(f"❌ Error reading {path}: {exc}")
...
if not USE_MONGODB:
    raise RuntimeError("❌ MongoDB is MANDATORY. Set USE_MONGODB=true in config.json")
...
if MONGODB_CONNECTION_STRING is None:
    raise RuntimeError("❌ MONGODB_CONNECTION_STRING is MANDATORY. Define it in .env file or config.json")

logger.info(f"✅ Unified API Server configured: {API_SERVER}")
logger.info("✅ MongoDB mode is MANDATORY - no fallback")
...
def init_storage():
    ...
    logger.info(f"🔄 Initializing MongoDB connection to: {MONGODB_DATABASE_NAME}")
    ...
    try:
        ...
        logger.info(f"✅ MongoDB store initialized: {MONGODB_DATABASE_NAME}")
        _mongodb_initialized = True

    except pymongo.errors.ServerSelectionTimeoutError:
        raise RuntimeError(f"❌ Cannot connect to MongoDB at {MONGODB_CONNECTION_STRING}. Is MongoDB running?")
