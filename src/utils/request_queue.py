...
class RequestQueue:
    ...
    async def add_request(
        ...
        """
        Add a request to queue with attachment data
        ...
        # Check if user already has requests in queue
        ...
        # Log attachment info
        if attachment_data:
            has_images = attachment_data.get("has_valid_images", False)
            has_files = attachment_data.get("has_text_files", False)
            if has_images:
                image_count = len([img for img in attachment_data.get("images", []) if not img.get("skipped")])
                logger.info(f"📎 Queueing request with {image_count} image(s) for user {user_id}")
            if has_files:
                file_count = len([f for f in attachment_data.get("text_files", []) if not f.get("skipped")])
                logger.info(f"📎 Queueing request with {file_count} text file(s) for user {user_id}")
    ...
    async def _worker(self):
        ...
        while True:
            try:
                ...
                if request.attachment_data:
                    has_images = request.attachment_data.get("has_valid_images", False)
                    if has_images:
                        image_count = len(
                            [img for img in request.attachment_data.get("images", []) if not img.get("skipped")])
                        logger.info(f"🖼️ Processing request with {image_count} image(s) for user {request.user_id}")
