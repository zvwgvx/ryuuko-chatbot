...
class QueuedRequest:
    ...
    ryuuko_user_id: str # Central System User ID
    is_owner: bool
    timestamp: float
    final_user_text: str
...
class RequestQueue:
    ...
    async def add_request(self, message: discord.Message, final_user_text: str, ryuuko_user_id: str) -> tuple[bool, str]:
        """
        ...
        user_has_queued_request = any(item.user_id == user_id for item in self._queue._queue)
        if user_has_queued_request:
            return False, "⏳ You already have a request in the queue. Please wait."
        ...
        if not is_owner:
            ...
            if current_time - last_request_time < 3.0:
                remaining = 3.0 - (current_time - last_request_time)
        ...
        logger.info(f"Adding request to queue for user {user_id} (Ryuuko ID: {ryuuko_user_id}): {final_user_text[:100]}")
