...
def setup_user_commands(bot: commands.Bot, user_config_manager, call_api, memory_store, mongodb_store):
    ...
    async def link_account_command(ctx: commands.Context, code: str):
        """Links your Discord account to your Ryuuko account using a code from the dashboard."""
        if not code:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                await ctx.send("✅ Your Discord account has been successfully linked!")
                logger.info(f"Successfully linked Discord account for user {ctx.author.id}.")

            except httpx.HTTPStatusError as e:
                error_detail = e.response.json().get("detail", "An unknown error occurred.")
                await ctx.send(f"❌ Linking failed: {error_detail}")
                logger.error(f"Failed to link account for {ctx.author.id}. API response: {error_detail}")
            except httpx.RequestError as e:
                await ctx.send("❌ Could not connect to the Ryuuko API. Please try again later.")
                logger.error(f"Could not connect to Ryuuko API at {api_url}: {e}")
