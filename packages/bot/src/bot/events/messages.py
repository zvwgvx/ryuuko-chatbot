...
REGISTRATION_MESSAGE = (
    "Chào bạn! Để bắt đầu trò chuyện, bạn cần có một tài khoản Ryuuko. "
    "Vui lòng đăng ký tại <https://ryuuko.app/register> và sau đó dùng lệnh `.link` để kết nối tài khoản Discord của bạn nhé."
)

# --- Attachment Processing Functions ---
# (Keep existing attachment functions as they are)
...
def split_message_smart(text: str, max_length: int = 2000) -> list[str]: return [text]
...
def setup_message_events(bot: commands.Bot, dependencies: dict):
    ...
    async def process_ai_request(request):
        ...
        ryuuko_user_id = request.ryuuko_user_id # Use the central ID

        try:
            # All data is now fetched using the ryuuko_user_id
            user_config = user_config_manager.get_user_config(ryuuko_user_id)
            user_model = user_config.get("model")
            user_system_message = {"role": "system", "content": user_config.get("system_prompt")}
            ...
            payload_messages.extend(memory_store.get_user_messages(ryuuko_user_id))
            ...
            if ok and resp:
                ...
                # Store history using the ryuuko_user_id
                memory_store.add_message(ryuuko_user_id, {"role": "user", "content": combined_text})
                memory_store.add_message(ryuuko_user_id, {"role": "model", "content": resp})

                if model_info and model_info.get("credit_cost", 0) > 0:
                    mongodb_store.deduct_user_credit(ryuuko_user_id, model_info["credit_cost"])
            elif not ok:
                await message.channel.send(f"⚠️ Lỗi: {str(resp)[:500]}", reference=message)

        except Exception as e:
            logger.exception(f"[ERROR] Lỗi khi xử lý request cho user {ryuuko_user_id}")
    ...
    async def on_message(message: discord.Message):
        ...
        # --- NEW: User Linking Check ---
        ryuuko_user_id = mongodb_store.get_ryuuko_user_id_from_discord_id(message.author.id)
        if not ryuuko_user_id:
            await message.channel.send(REGISTRATION_MESSAGE, reference=message)
            return
        # --- END: User Linking Check ---

        user_text = re.sub(rf"<@!?{bot.user.id}>", "", message.content or "").strip()
        # Pass the ryuuko_user_id to the queue
        success, msg = await request_queue.add_request(message, user_text, ryuuko_user_id)
        if not success:
