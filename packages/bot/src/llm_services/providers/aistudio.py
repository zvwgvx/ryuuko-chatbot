    ...
    if "messages" in data and isinstance(data["messages"], list):
        for msg in data["messages"]:
            if isinstance(msg, dict) and "role" in msg and "content" in msg:
                if msg["role"] == "system": # Bỏ qua system message ở đây vì đã thêm ở trên
                    continue
...
async def forward(request: Request, data: Dict, api_key: Optional[str]):
    """
    Chuyển tiếp yêu cầu đến API của Gemini bằng cách sử dụng AsyncOpenAI SDK.
    """
    ...
    if not key:
        return JSONResponse(
            {"ok": False, "error": "api_key_not_provided"},
            status_code=403,
        )
