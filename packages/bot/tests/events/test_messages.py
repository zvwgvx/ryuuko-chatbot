...
Refactored to align with the ryuuko_user_id authentication flow.
"""
import pytest
import discord
...
from src.bot.events.messages import setup_message_events, REGISTRATION_MESSAGE

# --- Constants ---
OWNER_ID = 99999
AUTHORIZED_USER_ID = 12345
UNLINKED_USER_ID = 54321
RYUUKO_USER_ID = "user_test_123"
...
def mock_bot():
    ...
    bot.listeners = {}
    bot.listen = Mock(side_effect=lambda event_name: lambda func: bot.listeners.update({event_name: func}) or func)
...
def mock_dependencies():
    request_queue = Mock()
    request_queue.add_request = AsyncMock(return_value=(True, None))
    request_queue.set_process_callback = Mock()

    mongodb_store = Mock()
    mongodb_store.get_ryuuko_user_id_from_discord_id.side_effect = lambda discord_id: RYUUKO_USER_ID if discord_id == AUTHORIZED_USER_ID else None
...
class TestOnMessageListener:
    ...
    async def test_ignores_bot_message(self, mock_bot, mock_dependencies, mock_message):
        ...
        await mock_bot.listeners['on_message'](mock_message)
        mock_dependencies['request_queue'].add_request.assert_not_called()

    async def test_rejects_unlinked_user(self, mock_bot, mock_dependencies, mock_message):
        mock_bot.get_context.return_value.valid = False
        mock_message.author.id = UNLINKED_USER_ID
        ...
        mock_dependencies['request_queue'].add_request.assert_not_called()
        mock_message.channel.send.assert_called_once_with(REGISTRATION_MESSAGE, reference=mock_message)

    async def test_accepts_linked_user_with_mention(self, mock_bot, mock_dependencies, mock_message):
        mock_bot.get_context.return_value.valid = False
        mock_message.mentions = [mock_bot.user]
        ...
        mock_dependencies['request_queue'].add_request.assert_called_once_with(mock_message, "Hello bot", RYUUKO_USER_ID)

@pytest.mark.asyncio
class TestProcessAIRequest:
    """Tests the core AI request processing logic with ryuuko_user_id."""
    ...
    def mock_request(self, mock_message):
        ...
        request.ryuuko_user_id = RYUUKO_USER_ID  # Set the central ID on the request
        return request
    ...
    async def test_successful_flow(self, mock_read_attachments, mock_send_long, mock_bot, mock_dependencies, mock_request):
        ...
        ucm.get_user_config.return_value = {"model": "test-model", "system_prompt": "System prompt", "access_level": 2, "credit": 100}
        ...
        # Assertions now use RYUUKO_USER_ID
        ucm.get_user_config.assert_called_once_with(RYUUKO_USER_ID)
        memory.get_user_messages.assert_called_once_with(RYUUKO_USER_ID)
        api.call_unified_api.assert_called_once()
        mock_send_long.assert_called_once_with(mock_request.message.channel, "This is the AI response.", mock_request.message)

        assert memory.add_message.call_count == 2
        memory.add_message.assert_any_call(RYUUKO_USER_ID, {"role": "user", "content": "This is a test prompt."})

        mongo.deduct_user_credit.assert_called_once_with(RYUUKO_USER_ID, 10)

    @patch('src.bot.events.messages._read_attachments_enhanced', new_callable=AsyncMock)
    async def test_insufficient_credits(self, mock_read_attachments, mock_bot, mock_dependencies, mock_request):
        mock_read_attachments.return_value = {"text_summary": "", "has_images": False}
        ucm = mock_dependencies['user_config_manager']
        ucm.get_user_config.return_value = {"model": "costly-model", "access_level": 2, "credit": 5}
