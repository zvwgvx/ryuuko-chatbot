...
This module provides a singleton `UserConfigManager` class that interfaces
with the MongoDB backend to store and retrieve user settings.
"""
import logging
...
# Default system prompt for new users or on configuration reset.
DEFAULT_SYSTEM_PROMPT = "Your name is Ryuuko (female), and you speak Vietnamese."
# Default model assigned to new users or on configuration reset.
DEFAULT_MODEL = "gemini-2.5-flash"

# Fallback list of supported models.
# This is used only if the list cannot be retrieved from MongoDB at startup.
...
class UserConfigManager:
    ...
    This manager is designed to work exclusively with a MongoDB backend.
    It raises a RuntimeError if the MongoDB store is not available upon initialization.
    """
...
def get_user_config_manager() -> UserConfigManager:
    ...
    This function ensures that only one instance of the manager is created
